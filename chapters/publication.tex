\clearpage
\phantomsection% 
\addcontentsline{toc}{chapter}{HALAMAN PERSETUJUAN PUBLIKASI}

\begin{center}
	% \small % Add this line to make the font smaller
	\smallskip

	\normalsize \bfseries \MakeUppercase{
		HALAMAN PERNYATAAN PERSETUJUAN PUBLIKASI \\
		TUGAS AKHIR UNTUK KEPENTINGAN AKADEMIS
	}\linebreak

	\normalfont \onehalfspacing \justifying % Changed \normalsize to \small
	\hspace{0.7cm}
	\begin{flushleft}
		Sebagai civitas akademik Institut Teknologi Sumatera, saya yang bertanda tangan di bawah ini:
	\end{flushleft}

	\flushleft
	\setlength{\tabcolsep}{0pt}
	\begin{tabular}{l l}
		Nama             & : \theauthor         \\
		NIM              & : \printnim          \\
		Program Studi \	 & : Teknik Informatika \\
		Fakultas         & : Teknologi Industri \\
	\end{tabular}
	\bigskip

	\justifying
	\noindent Demi pengembangan ilmu pengetahuan, menyetujui untuk memberikan kepada \textbf{Institut Teknologi Sumatera }\textit{Hak Bebas Royalti Noneksklusif} \textbf{(Non-exclusive Royalty Free Right)} atas karya ilmiah saya yang berjudul:\textbf{\thetitle} beserta perangkat yang ada (jika diperlukan). Dengan Hak Bebas Royalti Noneksklusif ini Institut Teknologi Sumatera berhak menyimpan, mengalihmedia/formatkan, mengelola dalam bentuk pangkalan data (\textit{database}), merawat, dan memublikasikan tugas akhir saya selama tetap mencantumkan nama saya sebagai penulis/pencipta dan sebagai pemilik Hak Cipta.
	\bigskip

	\noindent Demikian pernyataan ini saya buat dengan sebenarnya. \\

	\centering
	Lampung Selatan, \todayIndo\\
	% Pada tanggal : \todayIndo\\ % Format bulan harusnya nama panjang, belum kepikiran gimana caranya (RDB)
	\bigskip
	\vspace{1.5cm}
	\theauthor \\
	\printnim \\

\end{center}
\clearpage
